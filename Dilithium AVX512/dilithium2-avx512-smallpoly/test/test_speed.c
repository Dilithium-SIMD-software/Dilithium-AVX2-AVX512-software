...
int main(void)
{
  ...
  for(i = 0; i < NTESTS; ++i) {
    t[i] = cpucycles();
    crypto_sign_keypair_original(pk, sk);
  }
  print_results("Keypair original:", t, NTESTS);
  ...
  for(i = 0; i < NTESTS; ++i) {
    t[i] = cpucycles();
    crypto_sign_withouttailored(sm, &smlen, sm, CRHBYTES, sk);
  }
  print_results("Sign_withouttailored:", t, NTESTS);
   for(i = 0; i < NTESTS; ++i) {
    t[i] = cpucycles();
    crypto_sign_ntt(sm, &smlen, sm, CRHBYTES, sk);
  }
  print_results("Sign_ntt:", t, NTESTS);
  for(i = 0; i < NTESTS; ++i) {
    t[i] = cpucycles();
    crypto_sign_original(sm, &smlen, sm, CRHBYTES, sk);
  }
  print_results("Sign_original:", t, NTESTS);
  ...
  for(i = 0; i < NTESTS; ++i) {
    t[i] = cpucycles();
    crypto_sign_verify_original(sm, CRYPTO_BYTES, sm, CRHBYTES, pk);
  }
  print_results("Verify_original:", t, NTESTS);
  return 0;
}
